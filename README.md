...
| **External plan-review gate** | 🧪 **Durable backend contract verified with fake reviewers.** An opt-in task binds immutable rule evidence before specification, allocates a unique branch, sends the same validated snapshot to specification, implementation, corrections and final review, and refuses specification approval until every external finding is resolved. Coai tool names and schemas are fixed by a typed adapter; no real provider is contacted. |

Test suite: **1255 deterministic tests in 46 files, plus one automated Electron
...
* **The external plan gate has no renderer configuration yet.** INT-C's backend
  can bind rule evidence and enforce review before approval, but its explicit
  MCP executable, argv, convention selection and finding decisions are not yet
  exposed through the Electron UI. Ordinary tasks remain compatible and opt-in.
...
├─ tests/              1255 deterministic tests + 1 Electron E2E; no network, no real agents
